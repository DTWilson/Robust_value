...
for(i in 1:4){
  ...
  for(j in 1:nrow(df)){
    ...
    df$r_nct[j] <- max(regs[which(ns == df$n_nct[j]),])
  ...
  plots3a[[i]] <- ggplot(df, aes(k)) + geom_tile(aes(y = sig, fill = n_nct - n)) +
  ...
  plots3b[[i]] <- ggplot(df, aes(k)) + geom_tile(aes(y = sig, fill = (r_nct - r)/lambda)) +
...
So our analysis above follows if we have a fixed $\gamma$. If we assess recruitment rates at an interim analysis, this could lead to a revised estimate of $\gamma$. A slower than expected recruitment rate, for example, would correspond to a higher cost associated with $n$. Depending on how we weight the attributes $n$ and $t$, this re-estimate of $\gamma$ could lead to a larger change in our optimal design than any re-estimate of $\sigma$. 

To examine this we want a plot analogous to the one above where now we vary the true recruitment rate and look at the value of a fixed design (or designs, i.e. the simple one and the minimax one) and compare against the value of the locally optimal design.

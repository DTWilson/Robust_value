...
```{r}
pow <- function(n, delta, sig)
{
  suppressWarnings(1-pt(qt(0.975, 2*(n-1)), 2*(n-1), delta/sqrt(2*sig^2/n)))
}

v <- function(n, delta, sig)
{
  1-pt(qt(0.975, 2*(n-1)), 2*(n-1), delta/sqrt(2*sig^2/n)) - lambda*n
}

deltas <- seq(0, 0.5, length.out = 10^2)
sigs <- seq(0.5, 1.5, length.out = 10^2)
ns <- seq(2, 5, length.out = 10^2)
...
deltas <- seq(-1, 1.5, 0.01)
opt_ns <- NULL; opt_ps <- NULL
for(i in 1:length(deltas)){
  delta <- deltas[i]
  ps <- pow(ns, delta)
  vs <- v(ns, ps, delta)
  opt_ns <- c(opt_ns, ns[which.max(vs)])
  opt_ps <- c(opt_ps, ps[which.max(vs)])
}
plot(deltas, opt_ns)
points(deltas, 150*opt_ps, col="red")

x <- 200
deltas <- c(seq(-1, 0, 0.02), seq(0.3, 1.5, 0.02))#seq(-1, 1.5, 0.02)
regs <- NULL
for(i in 1:length(deltas)){
  delta <- deltas[i]
  ns <- 2:500
  ps <- pow(ns, delta)
  vs <- v(ns, ps, delta)
  regs <- c(regs, max(vs) - v(x, pow(x, delta), delta))
}
max(regs)

x <- 500
deltas <- seq(-1, 1.5, 0.02)
vs <- NULL
for(i in 1:length(deltas)){
  delta <- deltas[i]
  ps <- pow(x, delta)
  vs <- c(vs, v(x, ps, delta))
}
min(vs)
...
n1 <- 176; delta <- 0.3
pow(n1, delta)
lambda1 <- grad(pow, n1, delta = delta)
gamma1 <- 0.00001
...
A potentially useful extension of the above is to the SSR/pilot setting. The key difference is that we must move from value to utility, since the final attributes (sample size, power) will now be uncertain. We can stick with a frequentist framework, though.

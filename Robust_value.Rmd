...
```
For all cases, we find that the value of the optimal, simple, and minimax regret ns are pretty similar over the interval estimate of $\sigma$. This suggests that a fixed design can be very robust to misspecification of $\sigma$ (since moving to the optimal n would not improve value by much), and that the simple fixed design is almost as good a choice as the minimax regret design.
...
## SSR based on recruitment rates

One conclusion we can draw from the above analysis is that sample size re-estimation based on updated estimates of the standard deviation nuisance parameter aren't really worthwhile, since the resulting locally optimal design will have a similar value as the robust minimax regret design. Another thing which we might be able to re-estimate, though, is the recruitment rate of the trial. We can think about our value function as having both a sample size and a time component, e.g. in the power case

$$
v_1(n, \sigma; \lambda_1, c_1)  = \underbrace{1 - \Phi\left(z_{1 - \alpha} - \frac{\delta_a}{\sqrt{2\sigma^2/n}}\right)}_\textrm{Power} - \lambda_1 n - lambda_2 t c_1,
$$

where $t$ is the (expected) time taken to recruit the required $n$ patients. If the average time to recruit is $\gamma$, then we can rewrite the relevant component as

$$
- \lambda_1 n - \lambda_2 t = - (\lambda_1 + \lambda_2 \gamma) n,
$$

So our analysis above all follows if we have a fixed $\gamma$. If we assess recruitment rates at an interim analysis, this could lead to a revised estimate of $\gamma$. A slower than expected recruitment rate, for example, would correspond to a higher cost associated with $n$. Depending on how we weight the attributes $n$ and $t$, this re-estimate of $\gamma$ could lead to a much more considerable change in our optimal design than any re-estimate of $\sigma$. 

To examine this we want a plot analogous to the one above where now we vary the true recruitment rate and look at the value of a fixed design (or designs, i.e. the simple one and the minimax one) and compare against the value of the locally optimal design. The above reasoning would suggest the fixed designs would become suboptimal much faster as we move away from the initial recruitment rate estimate. This might be all we need to conclude that SSR based on recruitment rates might be much more useful than based on $\sigma$.
...
for(i in 1:4){
  ...
  lambda_o <- lambda/(1 + 2*lambda_rel/25)
  lambda <- lambda_o + 2*lambda_rel*lambda_o/df$gamma
  
  if(i < 3){
    df$v <- v1(n = df$n, sig = sig, lambda1 = lambda, c1 = C)
  } else {
    df$v <- v2(n = df$n, sig = sig, lambda2 = lambda, c2 = C)
  }
  ...
  # Get minimax regret n index
  #n_mm_ind <- which.min(apply(df_wide_reg[df_wide_reg$gamma > lo & df_wide_reg$gamma < up, 2:ncol(df_wide_reg)], 2, max))
  
  # Get minimax regret n  value at each sigma
  #v_mm <- df_wide[, 1 + n_mm_ind]
  
  # Get n using point estimate gamma = 0.156
  n_s_ind <- which.max(df_wide[df_wide$gamma == gamma_est, 2:ncol(df_wide)])
  ...
  plots2[[i]] <- ggplot(df3, aes(gamma, v, colour = t)) + geom_line() +
    scale_colour_manual(name = "", values = cols) +
    geom_vline(xintercept = c(lo, up), linetype = 2) +
...
Can we further extend to looking at retention? This will again affect the value function, now through impacting the power component. This is really just the same as adjusting the standard deviation though, which is what we end up doing here when implementing it to avoid having to change our value functions. So not really worth reporting, in comparison to the recruitment rate case which is different enough to be interesting.

```{r}
# First, choose a point estimate and construct a 95% CI around it
# tau = exp. proportion succesfully followed-up
tau_est <- 0.8
# For an estimated gamma, lower and upper CI limits are
lo <- tau_est - 1.96*sqrt(tau_est*(1-tau_est)/30)
up <- tau_est + 1.96*sqrt(tau_est*(1-tau_est)/30)
sig <- 0.8
...
for(i in 1:4){
  ...
  df <- expand.grid(tau = c(seq(0.6, 1, length.out = 50), tau_est),
                    n = 2:500)
  
  # Mimic the effect of loss to follow up by adjusting sigma
  sigs <- sig/df$tau
  
  if(i < 3){
    df$v <- v1(n = df$n, sig = sigs, lambda1 = lambda, c1 = C)
  } else {
    df$v <- v2(n = df$n, sig = sigs, lambda2 = lambda, c2 = C)
  }
  
  df_wide <- reshape(df, idvar = "tau", direction = "wide", timevar = "n", v.names = "v")
  ...
  # Get n using point estimate gamma = 0.156
  n_s_ind <- which.max(df_wide[df_wide$tau == tau_est, 2:ncol(df_wide)])
  
  # Get simple n value at each gamma
  v_s <- df_wide[, 1 + n_s_ind]
  
  df3 <- data.frame(tau = rep(df_wide$tau, 2),
                    v = c(v_opt, v_s),
                    t = c(rep(c("Optimal", "Simple"), each = length(df_wide$tau))))
  

  
  plots2[[i]] <- ggplot(df3, aes(tau, v, colour = t)) + geom_line() +
    scale_colour_manual(name = "", values = cols) +
    geom_vline(xintercept = c(lo, up), linetype = 2) +
    ylab("Value") + xlab("Retention") +
    theme_minimal()
}
...
```{r}
# Value function based on expected effectiveness - note cost of switching is
...
v3 <- function(n, sig, delta, lambda3, c3){
  ...
  (delta-0.1)*pow*prob_use - lambda3*n - c3
}
...
# Find the value of lambda s.t. the optimal sample size at point estimates
# sigma, delta is equal to n
obj_f <- function(lambda, n, sigma, delta){
  c3 <- lambda*n_cost
  abs( (which.max(v3(ns, sigma, delta, lambda, c3)) + 1) - n )
}
lamnda3 <- optim(0.0001, obj_f, n = 235, sigma = 1, delta = 0.3, 
      upper = 0.001, lower = 0.00001, method = "Brent")$par
# Check:
c3 <- lambda3*n_cost
...
# running example]
df3 <- df2[,1:2]
df3$n <- apply(df2[,3:ncol(df2)], 1, which.max) + 1
df3$v <- apply(df2[,3:ncol(df2)], 1, max)

ggplot(df3, aes(d, s)) + geom_tile(aes(fill = n)) +
    scale_fill_viridis(discrete=FALSE) +
    ylab("Standard deviation") + xlab("Effect size") +
    theme_minimal()
```

We see here that value is overwhelmingly determined by the true effect size, and not by the true standard deviation. This is not surprising since the former comes in directly and through power in the value function.
...
vs <- (delta-0.1)*(1-pt(qt(0.975, 2*(ns-1)), 2*(ns-1), delta/sqrt(2*sig^2/ns))) - lambda3*ns - c3

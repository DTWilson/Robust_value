...
pow(x = c(20,14), var_b = 0.021, p_0 = 0.7)
lambda <- grad(pow, c(20,14), var_b = 0.021, p_0 = 0.7)
gamma <- 2
  
df <- expand.grid(m = 2:50, n = c(14, 20))
df$var_b <- 0.021
df <- rbind(df, df)
  
df$p_0 <- c(rep(0.7, nrow(df)/2), rep(0.6, nrow(df)/2))
df$p <- apply(df, 1, function(z) pow(z[1:2], z[3], z[4]))

df$v <- apply(df, 1, function(z) v(z[1:2], z[5], lambda, gamma))

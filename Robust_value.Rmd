...
$$
\begin{align}
v_1(n; \lambda_1, c_1) & = \underbrace{1 - \Phi\left(z_{1 - \alpha} - \frac{\delta_a}{\sqrt{2\sigma^2/n}}\right)}_\textrm{Power} - \lambda_1 n - c_1 \\
...
for(i in 1:4){
  ...
  df <- expand.grid(sig = seq(0.5, 2, 0.1),
                    n = 2:500)
  if(i < 3){
    df$v <- v1(n = df$n, sig = df$sig, lambda1 = lambda, c1 = C)
  } else {
...
(plots2[[1]] + plots2[[3]]) /(plots2[[2]] + plots2[[4]]) + plot_layout(guides = "collect") & theme(legend.position = 'bottom')
```
...
for(i in 1:4){
  ...
  df3 <- data.frame(n = c(df$n[571], df$n_fix[571], df$n_ci[571]),
                    r = c(df$r[571], df$r_fix[571], df$r_ci[571]),
...
```{r}
(plots4[[1]] + plots4[[3]]) /(plots4[[2]] + plots4[[4]]) + plot_layout(guides = "collect") & theme(legend.position = 'bottom')
```

We find again that, for this specific point and interval example, the simple fixed n (where we maximise value based on the point estimate) is very close or identical to the minimax regret n and thus has a similar or identical maximum regret. This holds in all scenarios. In comparison, the upper CI n (where we choose the smallest n giving 80% power using the upper CI as the estimate) is always much higher and has a much larger maximum regret.
...
Seems to be a strong rationale for this approach - can clearly argue that the value of power at a fixed point is highly dependant on how likely we think that point is. E.g. very low value for power at unrealistically high points, except where mediated through implied power at realistic points (though this will be tending to 1).

```{r}
# Value function based on expected effectiveness
v3 <- function(n, sig, delta, lambda3, c3){
  p <- 1-pt(qt(0.975, 2*(n-1)), 2*(n-1), delta/sqrt(2*sig^2/n))
  delta*p - lambda3*n - c3
}

# Define space to get value over - corresponds to a specific interval estimates
sig <- 1; delta <- 0.25;  k <- 60
alpha <- 0.8
lo <- sqrt(k*sig^2/qchisq(alpha, k)) 
up <- sqrt(k*sig^2/qchisq(1-alpha, k)) 

deltas <- seq(delta - qnorm(alpha)*1/sqrt(k/2), delta + qnorm(alpha)*1/sqrt(k/2), length.out = 10^2)
...
lambda3 <- 0.0004; c3 <- lambda3*n_cost
...
# Plot value of optimal n at each parameter point [analogous to send plots in 
# running example]
df3 <- df2[,1:2]
...
# Plot value of value maximising n for delta = 0.25 and sig = 1 at each 
# parameter point 
vs <- delta*(1-pt(qt(0.975, 2*(ns-1)), 2*(ns-1), delta/sqrt(2*sig^2/ns))) - lambda3*ns - c3
fix_n <- ns[which.max(vs)]
df4 <- df3
df4$v <- df2[,1 + which.max(vs)]
fix_n
1-pt(qt(0.975, 2*(fix_n-1)), 2*(fix_n-1), delta/sqrt(2*sig^2/fix_n))

ggplot(df4, aes(d)) + geom_tile(aes(y = s, fill = v)) +
    scale_fill_viridis(discrete=FALSE) +
    theme_minimal()

# Plot difference in value between optimal and fixed ns
ggplot(df4, aes(d)) + geom_tile(aes(y = s, fill = df3$v - v)) +
    scale_fill_viridis(discrete=FALSE) +
    theme_minimal()

# Find the optimal (minimax regret) n
df5 <- df2 # Contains values
# For all ns, subtract value of best choice to give regrets
df5[,3:ncol(df5)] <- df3$v - df5[,3:ncol(df5)]
# For all ns, find maximum regret over all points
max_reg <- apply(df5[,3:ncol(df5)], 2, max)
# Find n which minmaxes regret
mm_reg_n <- ns[which.min(max_reg)]
mm_reg_n

# Plot value of minimax regret n at each parameter point
...
# Plot difference in value i.e. regret
ggplot(df5, aes(d)) + geom_tile(aes(y = s, fill = df3$v - v)) +
    scale_fill_viridis(discrete=FALSE) +
    theme_minimal()
```

Big difference here is that the simple fixed n (the one maximising value at the point estimates) is now quite larger than the minimax regret n. Since the value function parameters were chosen to give the former a power of 0.8, this means we have a minimax regret n which gives a power at that point of only 0.5. So we choose to have worse local value at the point, to get improved worst-case performance over the interval. If it seems undesiarable, we could change our interval to make it smaller.

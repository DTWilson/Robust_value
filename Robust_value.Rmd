...
Assuming a linear single attribute n, and assuming that the rate of substitution depends on $\beta$ but not on $n$ (so, the amount of $n$ we will pay to improve $\beta$ does not depend on $n$), a signle attribute value function for $\beta$ will complete our specification.
...
pow <- function(n, sig)
{
  ...
  1-pt(qt(0.975, 2*(n-1)), 2*(n-1), 0.3/sqrt(2*sig^2/n))
}

v <- function(n, p, lambda, gamma)
{
  # Value function
  -(log(p*gamma + 1)/log(gamma + 1) - lambda*n - lambda*20*(n > 0)) #0.3365181*(n > 0))
  # Adding a fixed penalty v(100, pow(100, 1), lambda, gamma),
  # So now for any value < 0 we are better to not run the trial
}

n1 <- 176; sig <- 1
pow(n1, sig)
lambda1 <- grad(pow, n1, sig=sig)
gamma1 <- 0.00001

#n2 <- 235; sig <- 1
n2 <- 110; sig <- 1
pow(n2, sig)
lambda2 <- grad(pow, n2, sig=sig)
gamma2 <- 2

params <- matrix(c(lambda1, gamma1,
                   0.00157, gamma2,
                   lambda2, gamma1,
                   0.00325, gamma2), ncol = 2, byrow = T)
...
for(i in 1:nrow(params)){
  ...
  df$p <- pow(df$n, df$sig)
  df$v <- v(df$n, df$p, lambda, gamma)
  
  df[which.min(df$v*(df$sig == 1)),]
  
  opt1 <- df[which.min(df$v*(df$sig == 1)),]
  opt2 <- df[which.min(df$v*(df$sig == 1.3)),]
  
  gr <- expand.grid(n = seq(4, 500, l = 10),
                    p = seq(0, 1, l = 10))
  gr$v <- v(gr$n, gr$p, lambda, gamma)
...
(plots[[1]] + plots[[2]]) /(plots[[3]] + plots[[4]]) + plot_layout(guides = "collect") & theme(legend.position = 'bottom')
```

For these four value scenarios, show how the true optimal n varies with the SD and compare against the constrained approach.
...
for(i in 1:nrow(params)){
  ...
  df$n <- sapply(df$sig, function(x) which.min(v(3:500, pow(3:500, x), lambda, gamma)) + 2)
  ...
  df2$n <- sapply(df$sig, function(x) which(pow(3:500, x) >= 0.8)[1] + 2)
  df2$t <- "c"
...
In all scenarios we see that sample size is fully or almost increasing, before we hit an SD that is so high the value of an optimal trial does not justify the set-up costs. It is interesting to see that for the linear cases, the optimal designs are not far off from those obtained by the constrained method.

Now, find the sample size which minimises the maximum regret over a given SD interval. For comparison, also look at upper CI and NCT methods.

First, make a list of scenarios in terms of estimated sds and their 95\% CI limits based on varying degrees of freedom / hypothetical pilot sample sizes.

```{r}
df <- expand.grid(sig = seq(0.3, 2, l = 10),
                  k = seq(10, 100, l = 10))
...
for(i in 1:nrow(params)){
  ...
  for(j in 1:nrow(df)){
    ...
    vals <- v(ns, pow(ns, df$sig[j]), lambda, gamma)
    vals[1] <- v(0, 0, lambda, gamma)
...
Now, look at a specific point estimate and interval. For each scenario, plot the values of (i) the optimal n, (ii) the minimax n, and (iii) the locally optimal n at the point estimate.
...
for(i in 1:nrow(params)){
  ...
  # Use the point estimate
  vals <- v(ns, pow(ns, sig), lambda, gamma)
  vals[1] <- v(0, 0, lambda, gamma)
...
Now, plot the minimax regret for each scenario for each n.
...
for(i in 1:nrow(params)){
  ...
  df2$t <- c("Minimax", "Fixed", "Upper CI", "NCT")
...
pows <- NULL
for(i in 1:1000){
  sig <- sqrt(t_var*rchisq(1, k)/k)
  #ns2 <- 3:500
  #z <- (2*qt(0.8, k, qt(0.975, 2*ns2 - 2))^2)*(sig^2)/(0.3^2)
  #n_nct <- ns2[ns2 >= z][1]
  n_nct <- power.t.test(delta = 0.3, sd = sig, power = 0.8)$n
  pows <- c(pows, pow(n_nct, sqrt(t_var)))
}
```

Although NCT paper is not particularly clear, I think the method lets us find an n(s) such that the power averaged over the sampling dist of s is nominal. So, as long as we are happy with this definition of power, this does provide a way to address uncertainty in sigma. But, it does nothing in terms of value and costs. So, best to view this as an extension to the usual constrained method, for the special case of a normal endpoint, for when we have uncertainty. 

The question is, should we be using their definition of power?

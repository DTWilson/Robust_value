...
When given a point and interval estimate of $\sigma$, we can choose $n$ to minimaise the maximum regret over that interval. 
...
[To do - repeat above getting the constrained n and / or the upper CI n, and plot the differences, so we can show the practical implications]
...
pow(m = 20, n = 14, var_b = 0.021, p_0 = 0.7)
lambda <- grad(function(z) pow(z[1], z[2], var_b = 0.021, p_0 = 0.7), c(20,14))
lambda <- c(0.013839575, 0.0008678032)
gamma <- 0.00001
  
df <- expand.grid(m = 2:50, n = c(7, 30))
df$p_0 <- 0.7
df <- rbind(df, df)
...
opt3 <- df[which.min(df$v - 10000*(df$n == 30 & df$var_b == 0.021)),]
opt4 <- df[which.min(df$v - 10000*(df$n == 30 & df$var_b == 0.042)),]
  
gr <- expand.grid(m = seq(2, 50, l = 10),
                  p = seq(0, 1, l = 10))
gr$v <- v(gr$m, n = 30, gr$p, lambda, gamma)
  
ggplot(df, aes(m, p)) + 
  geom_line(aes(colour=as.factor(var_b), linetype = as.factor(n))) +
    geom_hline(yintercept = 0.8, linetype=2) +
    geom_contour(data = gr, aes(z = v), alpha = 0.5, colour = cols[3]) +
    geom_contour(data = gr, aes(z = v), breaks = c(0), colour = cols[4]) +
    geom_point(data = df[as.numeric(row.names(rbind(opt1, opt2, opt3, opt4))), ]) +
    ylab("Power") + xlab("Sample size") +
    #scale_colour_manual(values=cols[1:2], labels=round(c(sig, 1.3*sig), 2)) +
...
sols[which.max(sols$max_r),]

ggplot(sols, aes(m, n, fill = max_r, z = max_r)) + 
  geom_tile() + geom_contour() +
  geom_point(data = sols[which.max(sols$max_r),]) +
  theme_minimal() +

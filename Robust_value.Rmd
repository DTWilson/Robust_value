...
opt$n <- sapply(opt$sig, function(x) optim(10, get_value, method="Brent", lower=2, upper=500, sig=x)$par)
opt$v <- apply(opt, 1, function(x) get_value(x[1],x[2]))

rob <- data.frame(n= 235, sig=seq(0.3,3,0.05))
rob$v <- apply(rob, 1, function(x) get_value(x[1], x[2]))
...
diff <- 0.01
o <- psoptim(100, coverage, opt=opt, diff=diff, lambda=lambda,
              lower = 2, upper = 500)
```

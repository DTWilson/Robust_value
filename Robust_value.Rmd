...
Consider a two-arm trial with a normal endpoint which will compare the means in each group via a t-test. We propose four example value functions
$$
v(n, 1-\beta)
$$
which define indifference curves over the space of sample sizes and powers.
...
for(i in 1:nrow(params)){
  ...
  df2$n <- sapply(df$sig, function(x) which(pow(3:500, x) >= (0.8*(i %in% c(1,2)) + 0.6*(i %in% c(3,4))))[1] + 2)
  df2$t <- "c"
